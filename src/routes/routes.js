...
// Create New Blog
router.post('/blog', [
    check('author').isLength({ min: 3, max: 20 }).isAlpha().withMessage("Author Name is required"),
    check('btitle').isLength({ min: 4, max: 50 }).isAlphanumeric().withMessage("Title is required"),
    check('bcontent').isLength({ min: 100 }).withMessage("Blog Content must greater than 100 words"),
